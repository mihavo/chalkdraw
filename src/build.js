...
/**
 * Resolve one variant's full palette: shells, syntax (with the dark ink
 * derivation applied), UI tokens and the git scale. Both the VS Code theme and
 * the Neovim colorscheme are generated from this, so the two editors can never
 * drift apart.
 */
...
/**
 * Emit lua/chalkdraw/palette.lua.
 *
 * Data only — the mapping from these roles onto highlight groups lives in
 * lua/chalkdraw/groups.lua, which is the Neovim counterpart of buildWorkbench().
 * Keeping the palette generated means a token change reaches both editors from
 * one edit, and the derived punctuation / line number can never diverge.
 */
function buildNeovimPalette() {
  const lua = [];
  lua.push('-- Generated by src/build.js from src/chalkdraw-tokens.json.');
  lua.push('-- Do not edit by hand: run `npm run build`.');
  lua.push('');
  lua.push('local M = {}');
  lua.push('');

  // 'function' and friends are Lua keywords, so any role name that collides has
  // to be written in bracket form or the file will not parse.
  ...
  const luaKey = (k) =>
    LUA_KEYWORDS.has(k) || !/^[A-Za-z_]\w*$/.test(k) ? `['${k}']` : k;
  const q = (v) => `'${v}'`;
  const tbl = (obj, indent) => {
    const pad = ' '.repeat(indent);
    return Object.entries(obj)
      .map(([k, val]) => `${pad}${luaKey(k)} = ${q(val)},`)
      .join('\n');
  };
  ...
  for (const key of Object.keys(tokens.variants)) {
    const { v, isDark, s, u, g } = resolvePalette(key);
    lua.push(`M.${key} = {`);
    lua.push(`  name = '${v.label}',`);
    lua.push(`  kind = '${isDark ? 'dark' : 'light'}',`);
    lua.push('  shell = {');
    lua.push(tbl({ editor: v.editorBackground, chrome: v.chromeBackground, line: v.currentLine }, 4));
    lua.push('  },');
    lua.push('  syntax = {');
    lua.push(tbl(s, 4));
    lua.push('  },');
  ...
  const out = path.join(ROOT, 'lua', 'chalkdraw', 'palette.lua');
...
{
  const out = buildNeovimPalette();
  const n = Object.keys(tokens.variants).length;
  console.log(`\u2713 Neovim palette \u2014 ${n} variants \u2192 ${path.relative(ROOT, out)}`);
}
